...
import { 
  ...
  Save, Plus, Trash2, ChevronDown, ChevronUp, AlertCircle, X, Loader, ArrowLeft 
} from 'lucide-react';
import Button from '../../components/Button';

export default function ProfileEditor({ profileId, onBack }) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await profilesApi.getProfile(profileId);
        const data = res.data;
        setProfileData({
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      await profilesApi.updateProfile(profileId, profileData);
  ...
  const handleAddEdu = () => {
    setProfileData(prev => ({
      ...prev,
      education: [...prev.education, { institution: '', degree: '', fieldOfStudy: '', startYear: new Date().getFullYear(), endYear: new Date().getFullYear() }]
    }));
  };
  ...
  const handleRemoveEdu = (index) => {
    setProfileData(prev => ({ ...prev, education: prev.education.filter((_, idx) => idx !== index) }));
  };
  ...
  const handleRemoveExp = (index) => {
    setProfileData(prev => ({ ...prev, experience: prev.experience.filter((_, idx) => idx !== index) }));
  };
  ...
  const handleAddProj = () => {
    ...
    setProfileData(prev => ({
      ...prev,
      projects: [...prev.projects, { title: '', description: '', technologies: [], githubLink: baseGit, liveLink: '' }]
    }));
  };
  ...
  const handleRemoveProj = (index) => {
    setProfileData(prev => ({ ...prev, projects: prev.projects.filter((_, idx) => idx !== index) }));
  };
  ...
  if (isLoading) {
    return <div className="flex justify-center p-12"><Loader size={24} className="animate-spin text-text-muted" /></div>;
  }
    ...
    <div className="flex flex-col gap-6 md:gap-8 w-full max-w-3xl animate-fade-in text-left">
      <div className="flex items-center gap-4 border-b border-brand-border pb-4">
        ...
        <div className="flex flex-col gap-1">
          <h2 className="font-heading text-2xl md:text-3xl font-bold text-text-main">Edit Profile</h2>
          <p className="text-xs md:text-sm text-text-muted">
            Customize the details for this specific profile.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSaveProfile} className="flex flex-col gap-5">
          ...
          {activeSection === 'skills' && (
            <div className="p-6 flex flex-col gap-4 bg-brand-bg animate-fade-in">
              <div className="flex gap-3">
                ...
                />
                <Button type="button" onClick={handleAddSkill} variant="secondary" className="py-3 px-5 shrink-0">
                  Add Skill
                </Button>
              </div>
              ...
              <div className="flex flex-wrap gap-2.5 mt-2">
                {profileData.skills.length === 0 ? (
                  ...
                  profileData.skills.map((skill, idx) => (
                    <span key={idx} className="inline-flex items-center gap-2 pl-3.5 pr-2 py-1.5 rounded-md bg-brand-surface border border-brand-border text-text-secondary text-xs font-semibold group hover:text-text-main transition-colors">
                      <span>{skill}</span>
                      <Button type="button" onClick={() => handleRemoveSkill(skill)} variant="ghost" size="sm" className="p-0.5! hover:text-red-400 hover:bg-red-500/10">
                        <X size={12} />
                      </Button>
                    </span>
                  ))
          ...
          {activeSection === 'education' && (
            <div className="p-6 flex flex-col gap-6 bg-brand-bg animate-fade-in">
              <div className="flex justify-between items-center">
                ...
                <Button type="button" onClick={handleAddEdu} variant="secondary" size="sm">
                ...
                <div className="flex flex-col gap-5">
                  {profileData.education.map((edu, index) => (
                    <div key={index} className="relative bg-brand-surface p-5 rounded-md border border-brand-border flex flex-col gap-5">
                      <Button type="button" onClick={() => handleRemoveEdu(index)} variant="ghost" size="sm" className="absolute top-4 right-4 p-1.5! hover:text-red-400 hover:bg-red-500/10">
                        <Trash2 size={14} />
                      </Button>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>School / University</label>
                          <input type="text" className={inputClass} value={edu.institution} onChange={(e) => handleEduChange(index, 'institution', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Degree / Certificate</label>
                          <input type="text" className={inputClass} value={edu.degree} onChange={(e) => handleEduChange(index, 'degree', e.target.value)} />
                        </div>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Field of Study</label>
                          <input type="text" className={inputClass} value={edu.fieldOfStudy} onChange={(e) => handleEduChange(index, 'fieldOfStudy', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Start Year</label>
                          <input type="number" className={inputClass} value={edu.startYear} onChange={(e) => handleEduChange(index, 'startYear', parseInt(e.target.value) || 0)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>End Year (Or Expected)</label>
                          <input type="number" className={inputClass} value={edu.endYear} onChange={(e) => handleEduChange(index, 'endYear', parseInt(e.target.value) || 0)} />
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeSection === 'experience' && (
            <div className="p-6 flex flex-col gap-6 bg-brand-bg animate-fade-in">
              <div className="flex justify-between items-center">
                ...
                <Button type="button" onClick={handleAddExp} variant="secondary" size="sm">
                  <Plus size={12} className="mr-1.5" />
                  <span>Add Work</span>
                </Button>
              </div>
              ...
              {profileData.experience.length === 0 ? (
                ...
                <div className="flex flex-col gap-5">
                  {profileData.experience.map((exp, index) => (
                    <div key={index} className="relative bg-brand-surface p-5 rounded-md border border-brand-border flex flex-col gap-5">
                      <Button type="button" onClick={() => handleRemoveExp(index)} variant="ghost" size="sm" className="absolute top-4 right-4 p-1.5! hover:text-red-400 hover:bg-red-500/10">
                        <Trash2 size={14} />
                      </Button>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Company Name</label>
                          <input type="text" className={inputClass} value={exp.company} onChange={(e) => handleExpChange(index, 'company', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Role Position</label>
                          <input type="text" className={inputClass} value={exp.position} onChange={(e) => handleExpChange(index, 'position', e.target.value)} />
                        </div>
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 items-center">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Location</label>
                          <input type="text" className={inputClass} value={exp.location} onChange={(e) => handleExpChange(index, 'location', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Start Date</label>
                          <input type="text" className={inputClass} value={exp.startDate} onChange={(e) => handleExpChange(index, 'startDate', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>End Date</label>
                          <input type="text" disabled={exp.current} className={`${inputClass} disabled:opacity-40`} value={exp.current ? 'Present' : exp.endDate} onChange={(e) => handleExpChange(index, 'endDate', e.target.value)} />
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <input id={`profile-job-${index}`} type="checkbox" className="w-4 h-4 rounded border-slate-500 bg-brand-bg text-text-main focus:ring-0 cursor-pointer" checked={exp.current} onChange={(e) => handleExpChange(index, 'current', e.target.checked)} />
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <textarea ref={(el) => { if (el) { el.style.height = 'auto'; el.style.height = `${el.scrollHeight}px`; } }} rows={3} className={`${inputClass} resize-none overflow-hidden`} value={exp.description} onKeyDown={handleTextareaKeyDown} onChange={(e) => { handleExpChange(index, 'description', e.target.value); e.target.style.height = 'auto'; e.target.style.height = `${e.target.scrollHeight}px`; }} />
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeSection === 'projects' && (
            <div className="p-6 flex flex-col gap-6 bg-brand-bg animate-fade-in">
              <div className="flex justify-between items-center">
                ...
                <Button type="button" onClick={handleAddProj} variant="secondary" size="sm">
                  <Plus size={12} className="mr-1.5" />
              ...
              {profileData.projects.length === 0 ? (
                ...
                <div className="flex flex-col gap-5">
                  {profileData.projects.map((proj, index) => (
                    <div key={index} className="relative bg-brand-surface p-5 rounded-md border border-brand-border flex flex-col gap-5">
                      <Button type="button" onClick={() => handleRemoveProj(index)} variant="ghost" size="sm" className="absolute top-4 right-4 p-1.5! hover:text-red-400 hover:bg-red-500/10">
                        <Trash2 size={14} />
                      </Button>
                      <div className="flex flex-col gap-2">
                        <label className={labelClass}>Project Title</label>
                        <input type="text" className={inputClass} value={proj.title} onChange={(e) => handleProjChange(index, 'title', e.target.value)} />
                      </div>
                      <div className="flex flex-col gap-2">
                        <label className={labelClass}>Project Description</label>
                        <textarea ref={(el) => { if (el) { el.style.height = 'auto'; el.style.height = `${el.scrollHeight}px`; } }} rows={2} className={`${inputClass} resize-none overflow-hidden`} value={proj.description} onKeyDown={handleTextareaKeyDown} onChange={(e) => { handleProjChange(index, 'description', e.target.value); e.target.style.height = 'auto'; e.target.style.height = `${e.target.scrollHeight}px`; }} />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Repository Link (GitHub)</label>
                          <input type="url" className={inputClass} value={proj.githubLink} onChange={(e) => handleProjChange(index, 'githubLink', e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2">
                          <label className={labelClass}>Live Preview Link</label>
                          <input type="url" className={inputClass} value={proj.liveLink} onChange={(e) => handleProjChange(index, 'liveLink', e.target.value)} />
                        </div>
                      </div>
                      <div className="flex flex-col gap-2">
                        <label className={labelClass}>Technologies Used (Comma-separated)</label>
                        <input type="text" className={inputClass} value={proj.technologies ? proj.technologies.join(', ') : ''} onChange={(e) => handleProjChange(index, 'technologies', e.target.value)} />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {/* Global Save Button */}
        <Button type="submit" disabled={isSaving} variant="primary" className="self-start mt-2">
          {isSaving ? (
            <><Loader size={16} className="animate-spin mr-2" /><span>Saving Changes...</span></>
          ) : (
            <><Save size={16} className="mr-2" /><span>Save Profile</span></>
          )}
        </Button>
      </form>
    </div>
