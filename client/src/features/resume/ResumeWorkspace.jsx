...
function ResumeWorkspace() {
  ...
  const isEditor = location.pathname.includes('/dashboard/resume/editor');

  return (
    <div className={`flex flex-col w-full animate-fade-in text-left ${isEditor ? 'h-full max-w-full gap-0 p-3 md:p-4' : 'gap-6 md:gap-8 max-w-7xl'}`}>
      ...
      <main key={location.pathname} className={`w-full flex-1 animate-fade-in transition-all duration-300 ease-in-out ${isEditor ? 'flex flex-col h-full overflow-hidden' : ''}`}>
        <Outlet />
      </main>
    </div>
  );
