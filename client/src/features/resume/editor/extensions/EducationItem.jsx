...
const EducationItemComponent = ({ node, updateAttributes, deleteNode, getPos, editor }) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-brand-border/50 pb-2">
          <div className="flex flex-col gap-1 w-full max-w-sm">
            ...
            <div className="flex items-center gap-1">
              <input 
                type="text"
                className="font-medium text-sm text-text-secondary bg-transparent border-none outline-none focus:ring-1 focus:ring-brand-primary/50 rounded px-1 w-1/3"
                placeholder="Degree"
              ...
              <input 
                type="text"
                className="font-medium text-sm text-text-secondary bg-transparent border-none outline-none focus:ring-1 focus:ring-brand-primary/50 rounded px-1 flex-1"
                placeholder="Field of Study"
                value={node.attrs.fieldOfStudy}
                onChange={(e) => updateAttributes({ fieldOfStudy: e.target.value })}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:items-end gap-1 shrink-0">
             <div className="flex items-center gap-2">
                <input 
                  ...
                  value={node.attrs.startYear}
                  onChange={(e) => updateAttributes({ startYear: e.target.value })}
                />
                ...
                <input 
                  ...
                  value={node.attrs.endYear}
                  onChange={(e) => updateAttributes({ endYear: e.target.value })}
                />
             </div>
          </div>
        </div>
...
export const EducationItem = Node.create({
  name: 'educationItem',
  group: 'block',
  content: 'block+',
  draggable: true,
  ...
  parseHTML() {
    return [
      { 
        tag: 'div[data-type="education-item"]',
        getAttrs: element => ({
          institution: element.getAttribute('institution'),
          degree: element.getAttribute('degree'),
          fieldOfStudy: element.getAttribute('fieldofstudy'),
          startYear: element.getAttribute('startyear'),
          endYear: element.getAttribute('endyear')
        })
      },
    ];
  },
  ...
  renderHTML({ HTMLAttributes }) {
    return ['div', mergeAttributes(HTMLAttributes, { 'data-type': 'education-item' }), 0];
  },

  addNodeView() {
    return ReactNodeViewRenderer(EducationItemComponent);
  },
});
